...
class ImageViewerView extends RelativeLayout
    ...
    // todo. Rewrite this method if we want to change onclick behavior of the photo slide
    private void onClick(MotionEvent event, boolean isOverlayWasClicked) {
        if (overlayView != null && !isOverlayWasClicked) {
            // if there is an overlay (eg description text overlaying the pictures, then just animate the text
            AnimationUtils.animateVisibility(overlayView);
            super.dispatchTouchEvent(event);
        } else {
            // dismiss the whole photo slide (which is an alert dialog) if there is no overlay
            onDismiss();
        }
    }
